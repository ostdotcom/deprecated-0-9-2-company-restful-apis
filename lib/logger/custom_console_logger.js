...
CustomConsoleLoggerKlass.prototype = {
   ...
   */
  notify: function (code, msg, data, params, backtrace) {
    const oThis = this
    ;

    // convert the custom error object to formatted object.
    if (responseHelper.isCustomResult(data)) {
      let formattedError = data.toHash();
      formattedError.debug_options = data.debug_options;
