...
require(rootPrefix + '/lib/cache_management/managedAddressesSalt');
...
AddressEncryptorKlass.prototype = {
  ...
  encrypt: async function(dataToEncrypt) {
    ...
    var response = await oThis._fetchAddressSalt(oThis.managedAddressSaltId);

    if (response.isFailure()) {
      return Promise.resolve(null);
    }

    var encrypt = crypto.createCipher(algorithm, response.data.addressSalt);
    var theCipher = encrypt.update(dataToEncrypt, 'utf8', 'hex');
  ...
  decrypt: async function(encryptedData) {
    ...
    var decipher = crypto.createDecipher(algorithm, response.data.addressSalt);
  ...
  _fetchAddressSalt: async function() {
    const oThis = this,
      addressSaltCacheKlass = oThis.ic().getManagedAddressSaltCache();

    var obj = new addressSaltCacheKlass({ id: oThis.managedAddressSaltId });
...
InstanceComposer.registerShadowableClass(AddressEncryptorKlass, 'getAddressEncryptorClass');
