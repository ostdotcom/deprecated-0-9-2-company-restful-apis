...
TransactionFormatterKlass.prototype = {
  ...
  perform: async function() {
    ...
    if (oThis.params.block_number) {
      ...
      let amount = oThis.params.amount_in_wei
        ? basicHelper.convertToNormal(oThis.params.amount_in_wei).toString(10)
        : null;

      let commission = oThis.params.commission_amount_in_wei
        ? basicHelper.convertToNormal(oThis.params.commission_amount_in_wei).toString(10)
        : null;

      let airdropAmount = oThis.params.airdrop_amount_in_wei
        ? basicHelper.convertToNormal(oThis.params.airdrop_amount_in_wei).toString(10)
        : null;
