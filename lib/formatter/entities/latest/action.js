...
ActionFormatterKlass.prototype = {
  perform: async function() {
    ...
    if (currency == clientTxTypesConst.usdCurrencyType && !commonValidator.isVarNull(oThis.params.value_in_usd)) {
      ...
      currency == clientTxTypesConst.btCurrencyType &&
      !commonValidator.isVarNull(oThis.params.value_in_bt_wei)
    ) {
      amount = basicHelper.convertToNormal(oThis.params.value_in_bt_wei).toString(10);
    }
    ...
    formattedAction.commission_percent =
      kind == clientTxTypesConst.userToUserKind ? oThis.params.commission_percent : null;

    formattedAction.arbitrary_commission =
      kind == clientTxTypesConst.userToUserKind ? commonValidator.isVarNull(oThis.params.commission_percent) : null;
