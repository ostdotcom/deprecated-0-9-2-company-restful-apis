...
ActionFormatterKlass.prototype = {
  ...
  perform: async function(){
    ...
    const oThis = this
      , mandatoryRootLevelKeys = ['id', 'client_id', 'name', 'kind', 'currency_type']
      , formattedAction = {}
    ;
    ...
    var amount = null
      , kind = new ClientTransactionTypeModel().kinds[oThis.params.kind].toUpperCase();

    if(kind == clientTxTypesConst.usdCurrencyType && !commonValidator.isVarNull(oThis.params.value_in_usd)){
      amount = oThis.params.value_in_usd;
    } else if (kind == clientTxTypesConst.btCurrencyType && !commonValidator.isVarNull(oThis.params.value_in_bt_wei)) {
    ...
    formattedAction.currency = new ClientTransactionTypeModel().currencyTypes[oThis.params.currency_type];
    ...
    formattedAction.arbitrary_commission = commonValidator.isVarNull(oThis.params.commission_percent);
