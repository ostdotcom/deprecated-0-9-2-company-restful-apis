...
economyUserBalance.prototype = {
  ...
  perform: async function() {
    ...
    if (clientBrandedTokenResponse.isFailure()) {
      notifier.notify(
        'l_eub_1',
        'Invalid Client branded token response for multiget fetch',
    ...
    if (airdropBalanceResponse.isFailure()) {
      notifier.notify('l_eub_2', 'Error in getAirdropBalance', airdropBalanceResponse, {
        clientId: oThis.clientId,
    ...
    if (fetchBalanceObjResponse.isFailure()) {
      notifier.notify('l_eub_4', 'Error in fetching balance for ethereum addresses', fetchBalanceObjResponse, {
        clientId: oThis.clientId,
        ethereum_address: oThis.ethereumAddresses
      });
