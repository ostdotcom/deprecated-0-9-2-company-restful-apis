...
economyUserBalance.prototype = {
  ...
  perform: async function () {
    ...
    if (clientBrandedTokenResponse.isFailure()) {
      logger.notify('l_eub_1', 'Invlaid Client branded token response for multiget fetch',
          clientBrandedTokenResponse, {clientId: oThis.clientId});
    ...
    if (airdropBalanceResponse.isFailure()) {
      logger.notify('l_eub_2', 'Error in getAirdropBalance for ' + oThis.ethereumAddresses,
          airdropBalanceResponse, {clientId: oThis.clientId});
    ...
    for (var i in oThis.ethereumAddresses) {
      ...
      if ((!platformBalanceResponse) || platformBalanceResponse.isFailure()) {
        logger.notify('l_eub_3', "Error in fetching balance of Address - " + ethereumAddress,
            platformBalanceResponse, {clientId: oThis.clientId});
