...
const rootPrefix = '../..'
  ...
  , gethProviders = JSON.parse(process.env.OST_GETH_RPC_PROVIDERS); // This should be moved to appropriate location ?, Also while code review check what should be the env variable key name

/**
 * @constructor
 *
 * @param {Object} params - cache key address, chain_kind, chain_id
 *
 */
const NonceCacheKlass = function(params) {
...
const NonceCacheKlassPrototype = {
    ...
    return new Promise(async function(onResolve, onReject) {
      ...
      var wait = async function() {
        try {
          ...
          if (isLocked) {
            setTimeout(wait, 1000); // 10 milliseconds   
          } else {          
  ...
  _syncNonce: async function(){
    ...
    const allGethNodes = gethProviders[oThis.chainId];
    ...
    if (isNonceValid) {
      const setNonceResponse = await oThis.cacheImplementer.set(oThis.cacheKey, maxNonceCount.toString());
      if (setNonceResponse.isSuccess()) {
        return responseHelper.successWithData({nonce: maxNonceCount.toString()});
      }
      ...
      try {
        web3Provider.eth.getTransactionCount(oThis.address, 'pending',function(error, result) {
          if (error) {
            ...
            return onResolve(responseHelper.successWithData({nonce: result.toString()}));
          }         
        });
      } catch (err) {
        //Format the error
...
Object.assign(NonceCacheKlass.prototype, NonceCacheKlassPrototype);

module.exports = NonceCacheKlass;
