...
ApproveKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if (responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_sam_a_1", "Inside catch block", {error: error},
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_a_3", "criticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_a_4", "parentCriticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }
  ...
  approve: async function () {
    ...
    if (approveResponse.isFailure() || !transactionReceipt || parseInt(transactionReceipt.status, 16) !== 1) {
      return Promise.reject(responseHelper.error("l_snm_a_2", 'approve failed',
