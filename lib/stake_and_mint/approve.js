    ...
    return oThis.asyncPerform()
        .catch(function (error) {
          ...
          if(responseHelper.isCustomResult(error)) {
            ...
            errorObj = responseHelper.error("l_snm_a_1", "Inside catch block", null, {error: error}, {sendErrorEmail: true});
  ...
  asyncPerform: async function () {
    ...
    if (approveResponse.isFailure() || !transactionReceipt || parseInt(transactionReceipt.status, 16) !== 1) {
      errorObj = responseHelper.error(
          "l_snm_a_2", 'approve failed', null, {err: approveResponse.err || {}, data: approveResponse.data || {}},
          {sendErrorEmail: true}
      );
      return Promise.reject(errorObj);
    }

    var response = responseHelper.successWithData(approveResponse.data || {});
