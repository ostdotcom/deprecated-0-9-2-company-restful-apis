...
VerifyTransferToStakerKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_sam_vtts_1", "Inside catch block", {error: error},
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_vtts_2", "criticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_vtts_3", "parentCriticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }
    ...
    return new Promise(function(onResolve, onReject) {
      ...
      const getReceiptAttempt = async function() {
        ...
        if (max_retry_count > 0){
          ...
          return onReject(responseHelper.error("l_sam_vtts_6", "Receipt not successful after maximum retries", {},
            {sendErrorEmail: false}));
        }
      };
  ...
  validateReceipt: async function(tx_receipt_data){
    ...
    if (clientEthAddress != encryptedEthAddress){
      return Promise.reject(responseHelper.error("l_sam_vts_5", "Invalid From Address", {},
        {sendErrorEmail: true, clientId: oThis.clientId}));
    }

    if (rawReceipt.to.toLowerCase() != chainInteractionConstants.SIMPLE_TOKEN_CONTRACT_ADDR.toLowerCase()){
      return Promise.reject(responseHelper.error("l_sam_vts_3", "Invalid To Address", {},
        {sendErrorEmail: true, clientId: oThis.clientId}));
    }
    ...
    if (buffer[0]['value'].toLowerCase() != chainInteractionConstants.STAKER_ADDR.toLowerCase()){
      return Promise.reject(responseHelper.error("l_sam_vts_4", "Invalid transfer To Address", {},
        {sendErrorEmail: true, clientId: oThis.clientId}));
    }
