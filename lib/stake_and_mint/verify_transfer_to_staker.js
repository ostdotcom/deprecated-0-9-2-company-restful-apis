...
VerifyTransferToStakerKlass.prototype = {
  ...
  validateReceipt: async function(tx_receipt_data){
    ...
    let clientEthAddress = oThis.criticalChainInteractionLog.request_params.client_eth_address.toLowerCase();

    if (clientEthAddress != rawReceipt.from){
    ...
    let formattedReceipt = rawReceipt.formattedTransactionReceipt;
  ...
  updateCriticalInteractionLog: async function () {
    ...
    await new CriticalChainInteractionLogModel().updateCriticalChainInteractionLog(
      oThis.parentCriticalInteractionLogId,
      {
        request_params: JSON.stringify(oThis.parentCriticalChainInteractionLog.request_params)
      },
      oThis.parentCriticalInteractionLogId,
      oThis.clientTokenId
    );
