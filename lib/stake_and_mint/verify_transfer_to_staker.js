...
VerifyTransferToStakerKlass.prototype = {
  ...
  validateReceipt: async function(tx_receipt_data){
    ...
    if (clientEthAddress != encryptedEthAddress){
      return Promise.reject(responseHelper.error("l_sam_vts_5", "Invalid From Address", null, {},
        {sendErrorEmail: true}));
    }

    if (rawReceipt.to.toLowerCase() != chainInteractionConstants.SIMPLE_TOKEN_CONTRACT_ADDR.toLowerCase()){
      return Promise.reject(responseHelper.error("l_sam_vts_3", "Invalid To Address", null, {},
        {sendErrorEmail: true}));
    }
    ...
    if (buffer[0]['value'].toLowerCase() != chainInteractionConstants.STAKER_ADDR.toLowerCase()){
      return Promise.reject(responseHelper.error("l_sam_vts_4", "Invalid transfer To Address", null, {},
        {sendErrorEmail: true}));
    }
