...
const GetTransferToStakerStatusKlass = function(params){
  ...
  oThis.parent_id = params.parent_critical_interaction_log_id;
 ...
 *
 * @Prototype {{
 * perform: GetTransferToStakerStatusKlass.perform,
 * asyncPerform: GetTransferToStakerStatusKlass.asyncPerform,
 * validateAndSanitize: GetTransferToStakerStatusKlass.validateAndSanitize,
 * getStakerTransferStatus: GetTransferToStakerStatusKlass.getStakerTransferStatus,
 * validateReceipt: GetTransferToStakerStatusKlass.validateReceipt,
 * }}
 */
...
GetTransferToStakerStatusKlass.prototype = {
  ...
  validateAndSanitize: async function() {
    ...
    res = criticalChainInteractionLogObj.getByIds([oThis.critical_log_id])[oThis.critical_log_id];
    ...
    oThis.st_prime_to_mint = oThis.critical_chain_interaction_log.request_params.stake_and_mint_params.st_prime_to_mint;

    return Promise.resolve(responseHelper.successWithData({}));
  },

  /**
   * getStakerTransferStatus
   * @returns {Promise<resolve>}
   */
  ...
  validateReceipt: async function(tx_receipt_data){
    ...
    let buffer = formatted_receipt.eventsData[0].events.filter(function(event) {
    ...
    buffer = formatted_receipt.eventsData[0].events.filter(function(event) {
    ...
    oThis.critical_chain_interaction_log.request_params.stake_and_mint_params.ost_to_stake_to_mint_bt = ost_to_stake_to_mint_bt;
