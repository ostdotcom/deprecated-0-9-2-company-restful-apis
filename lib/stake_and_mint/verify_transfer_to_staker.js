...
class GetTransferToStakerStatus {
  constructor(critical_log_id,
              parent_id,) {
    const oThis = this;

    oThis.critical_log_id = critical_log_id;
    oThis.parent_id = parent_id;
  ...
  async asyncPerform() {
    ...
    res = await oThis.getStakerTransferAddress();
    if (res.isFailure()) return Promise.resolve(res);
  ...
  async validateAndSanitize() {
    ...
    res = CriticalChainInteractionLogModel.getByIds([oThis.critical_log_id])[0];
  ...
  async getStakerTransferAddress() {
    const oThis = this;
    var res = null;

    if (oThis.critical_chain_interaction_log.status == criticalChainInteractionLogConst.processedStatus){
      return Promise.resolve(responseHelper.successWithData({}));
    }
    ...
    res = await CriticalChainInteractionLogModel
      .update({
  ...
  async validateReceipt(tx_receipt_data){
    const oThis = this;
