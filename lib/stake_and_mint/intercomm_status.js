...
IntercommStatusKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if (responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_sam_is_1", "Inside catch block", {error: error},
            {sendErrorEmail: true});
        }
  ...
  asyncPerform: async function () {
    ...
    if (oThis.status.includes('failed')) {
      ...
      return Promise.reject(responseHelper.error("l_sam_is_2", "Stake and mint Processor returned error",
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if(!manageAddressRecord){
      return Promise.reject(responseHelper.error("l_sam_is_3", "manageAddressRecord not found",
        {reserveAddress: reserveAddress}, {sendErrorEmail: false}));
    }
    const clientBrandedToken = (await new ClientBrandedTokenModel().select('*')
      .where({reserve_managed_address_id: manageAddressRecord.id}).fire())[0];

    if(!clientBrandedToken){
      return Promise.reject(responseHelper.error("l_sam_is_4", "clientBrandedToken not found",
    ...
    if(!oThis.criticalChainInteractionLog){
      return Promise.reject(responseHelper.error("l_sam_is_5", "oThis.criticalChainInteractionLog not found",
