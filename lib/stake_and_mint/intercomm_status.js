...
const IntercommStatusKlass = function(params) {
  ...
  oThis.tokenType = params.tokenType;
...
IntercommStatusKlass.prototype = {
  ...
  asyncPerform: async function() {
    ...
    if (oThis.status == 'claim_token_on_uc_done' && oThis.tokenType == 'bt') {
      let ethAddrPrivateKeyCacheResponse = await new EthAddrPrivateKeyCacheKlass({
          address: oThis.benificiaryAddress
        }).fetch(),
        clientId = ethAddrPrivateKeyCacheResponse.data.client_id,
        response = await configStrategyHelper.getConfigStrategy(clientId),
        instanceComposer = new InstanceComposer(response.data),
