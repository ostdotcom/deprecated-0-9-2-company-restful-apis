...
BaseKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if (responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_sam_s_b_1", "Inside catch block", {error: error},
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_s_b_2", "criticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_sam_s_b_3", "parentCriticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }
