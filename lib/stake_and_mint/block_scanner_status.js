...
BlockScannerStatusKlass.prototype = {
      ...
      .catch(async function (error) {
        ...
        if (oThis.criticalChainInteractionLog) {
          await new CriticalChainInteractionLogModel().updateCriticalChainInteractionLog(
            ...
            logger.error('lib/stake_and_mint/block_scanner_status.js::perform::catch::updateCriticalChainInteractionLog');
            logger.error(err);
          });
        }
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    oThis.criticalChainInteractionLog = (await new CriticalChainInteractionLogModel().select('*')
      .where(['client_id=? AND activity_type=?', oThis.client_id, activityType]).order_by('id DESC')
      .limit(1).fire())[0];
