...
StakeAndMintRouterKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(function(error) {
        if (responseHelper.isCustomResult(error)){
          ...
          return responseHelper.error("l_ob_r_1", "Unhandled result", {}, {});
        }
      });
  },
  ...
  asyncPerform: async function () {
    ...
    if(['st_prime_stake_and_mint_complete', 'bt_stake_and_mint_complete'].includes(oThis.currentStep)
      && oThis.status === 'done' && oThis.performAirdrop) {
  ...
  init: async function () {
    ...
    if(!oThis.parentCriticalInteractionLogId) {
      const clientBrandedTokenRecords = await new ClientBrandedTokenModel().select('id')
        .where({symbol: oThis.tokenSymbol}).fire();

      const clientBrandedToken = clientBrandedTokenRecords[0];

      if(!clientBrandedToken){
        return Promise.reject(responseHelper.error('l_ob_r_2', 'no client branded token found for symbol'+oThis.tokenSymbol,
      ...
      const criticalChainInteractionLogs = await new CriticalChainInteractionLogModel().getByIds([oThis.parentCriticalInteractionLogId])
      ...
      if (!parentCriticalChainInteractionLog) {
        const errorRsp = responseHelper.error("l_ob_r_3", "parentCriticalChainInteractionLog not found", {}, {sendErrorEmail: false});
    ...
    const queueBackgroundJobResponse = await oThis.queueBackgroundJob(activityType,
      notificationTopics.stakeAndMintInitTransfer, requestParams, oThis.transactionHash);
  ...
  queueBackgroundJob: async function (activityType, topic, requestParams, transactionHash) {
    ...
    if (activityType === new CriticalChainInteractionLogModel().invertedActivityTypes[
            criticalChainInteractionLogConst.stakerInitialTransferActivityType]) {

      chainType = new CriticalChainInteractionLogModel().invertedChainTypes[criticalChainInteractionLogConst.valueChainType]
