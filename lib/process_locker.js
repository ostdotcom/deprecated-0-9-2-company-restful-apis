...
ProcessLockerKlass.prototype = {
  ...
  canStartProcess: function(params) {
    ...
    let processID = (shell.exec("ps -ef | grep '" + processTitle + "' | grep -v grep | awk '{print $2}'") || {}).stdout;
    if (processID === '') {
      // Title for Process
   ...
   *
   * @param {Object} params - parameters
   * @param {number} params.time_in_minutes - Time Interval after which process has to be terminated
   *
   */
