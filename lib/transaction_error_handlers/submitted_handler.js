...
const rootPrefix = '../..',
  ...
  configStrategyConst = require(rootPrefix + '/lib/global_constant/config_strategy'),
...
const SubmittedHandlerKlassPrototype = {
  ...
  asyncPerform: async function() {
    ...
    // transactionsWithSubmittedStuck has transactions.
    if (oThis.transactionsWithSubmittedStuck.length > 0) {
      oThis._notifyDevelopers();
      await oThis._releaseLock(oThis.transactionsWithSubmittedStuck, true);
    }
    ...
    if (oThis.transactionHashesToBeResubmitted.length > 0) {
      ...
      await oThis._resubmitTransaction();

      await oThis._setNextActionAt();
      await oThis._clearCache();
      await oThis._releaseLock(oThis.transactionHashesToBeResubmitted, true);
    }
  },
  ...
  _createConfigStrategyAndObjectsForClients: async function() {
    ...
    console.log('---oThis.clientTxHashMapping----', oThis.clientTxHashMapping);
    console.log('---oThis.txUuidClientMapping----', oThis.txUuidClientMapping);
    console.log('---oThis.txHashAndUuidMapping----', oThis.txHashAndUuidMapping);
    ...
    for (let index = 0; index < clientIds.length; index++) {
      ...
      const configStrategy = configStrategyResp.data,
        ...
        transactionLogCache = ic.getTransactionLogCache();
      ...
      oThis.clientRelatedMapping[clientId].transactionLogCache = transactionLogCache;
    ...
    console.log('---oThis.clientRelatedMapping---', oThis.clientRelatedMapping);
  },
  ...
  _getTransactionsFromGeth: async function() {
    ...
    for (let clientId in oThis.clientRelatedMapping) {
      if (oThis.clientRelatedMapping.hasOwnProperty(clientId)) {
        ...
        console.log('---clientTxHashes---', clientTxHashes);
        ...
        for (let index = 0; index < clientTxHashes.length; index++) {
          ...
          logger.win('--transactionDetails---', transactionDetails);
          ...
          // Found transactions stuck with Submitted status in the DB but are available on geth.
            ...
            oThis.clientRelatedMapping[clientId].transactionHashesToBeResubmitted.push(transactionHash); //TODO: Remove this line.
          }
        }
      }
    }

    //oThis.transactionHashesToBeResubmitted = oThis.transactionsWithSubmittedStuck;

    console.log('---oThis.transactionHashesToBeResubmitted---', oThis.transactionHashesToBeResubmitted);
    console.log('---oThis.transactionsWithSubmittedStuck---', oThis.transactionsWithSubmittedStuck);
    console.log('---oThis.clientRelatedMapping---', oThis.clientRelatedMapping);
  },
  ...
  _fetchFromTransactionLog: async function() {
    ...
    console.log('---oThis.transactionFetchResponse---', oThis.transactionFetchResponse);
    return Promise.resolve(oThis.transactionFetchResponse);
  },
  ...
  _resubmitTransaction: async function() {
    ...
    let transactionsToBeResubmitted = oThis.transactionFetchResponse[0].data; // Returns a map.
    // Loop over all the transactions found.
    for (let transaction in transactionsToBeResubmitted) {
      if (transactionsToBeResubmitted.hasOwnProperty(transaction)) {
        // Get rawTx and gethUrl.
        let rawTx = JSON.parse(transaction.raw_transaction),
          clientId = oThis.txUuidClientMapping[transaction],
          gethUrl = oThis.clientRelatedMapping[clientId].gethUrl;

        let resubmitRawTxObj = new ResubmitRawTx(rawTx, gethUrl);
        resubmitRawTxObj.perform();
      }
    }
  },

  /**
   * Clears cache for the particular transaction uuids belonging to a specific client.
   *
   * @param {String} uuid
   ...
   * @private
   */
  _clearCache: async function(uuid, clientId) {
    const oThis = this;

    await new oThis.clientRelatedMapping[clientId].transactionLogCache({
      uuids: [uuid],
      client_id: clientId
    }).clear();
  },
   ...
   * Release lock.
   *
   * @param {Array} transactionHashesToBeReleased
   * @param {Boolean} shouldUpdate
   * @returns {Promise<void>}
   * @private
   */
  _releaseLock: async function(transactionHashesToBeReleased, shouldUpdate) {
    const oThis = this;

    const nextActionAtTimeStamp =
        Math.floor(Date.now() / 1000) + transactionMetaConst.statusActionTime[transactionMetaConst.submitted],
      whereOptions = ['transaction_hash IN (?)', transactionHashesToBeReleased];

    if (shouldUpdate) {
      const updateOptions = ['next_action_at = ? AND retry_count = retry_count + 1', nextActionAtTimeStamp];
      await new TransactionMetaModel().releaseLock(oThis.lockId, whereOptions, updateOptions);
    } else {
      await new TransactionMetaModel().releaseLock(oThis.lockId, whereOptions);
    }
  }
};
