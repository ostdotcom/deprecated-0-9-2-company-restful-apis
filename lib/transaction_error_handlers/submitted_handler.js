...
const SubmittedHandlerKlassPrototype = {
  ...
  asyncPerform: async function() {
    ...
    if (oThis.transactionHashesToBeResubmitted.length > 0) {
      await oThis._resubmitUnknownTransactions();
    }
  },
  ...
  _fetchFromTransactionLog: async function(clientId, transactionUuids) {
    ...
    let txLogResponse = await new oThis.clientRelatedMapping[clientId].transactionLogModel({
      client_id: clientId,
   ...
   * @private
   */
  _resubmitUnknownTransactions: async function() {
    const oThis = this,
      batchSize = 50;
