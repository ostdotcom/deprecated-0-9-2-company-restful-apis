 ...
 *
 * NOTES:
 * 1. Batching of transaction meta rows is done by transaction meta handler cron.
 *
...
const SubmittedHandlerKlassPrototype = {
  ...
  perform: function() {
    ...
    return oThis.asyncPerform().catch(function(error) {
      // Release lock on all rows if exception occurs.
      new TransactionMetaModel().releaseLock(oThis.lockId);
  ...
  asyncPerform: async function() {
    ...
    if (oThis.invalidTxIds.length > 0) {
      ...
      await oThis._releaseLock(oThis.invalidTxIds);
    ...
    for (let clientId in oThis.clientRelatedMap) {
      ...
      for (let index = 0; index < clientTxHashes.length; index++) {
        ...
        let transactionHash = clientTxHashes[index],
          transactionDetails = await oThis.clientRelatedMap[clientId].web3Interact.getTransaction(transactionHash);

        oThis.clientRelatedMap[clientId].transactionHashesToBeResubmitted =
          oThis.clientRelatedMap[clientId].transactionHashesToBeResubmitted || [];
        ...
        if (!transactionDetails) {
          ...
          oThis.clientRelatedMap[clientId].transactionHashesToBeResubmitted.push(transactionHash);
        }
  ...
  _resubmitTransaction: async function(transactionsToBeResubmitted) {
    ...
    for (let transactionUuid in transactionsToBeResubmitted) {
      ...
      if (transactionsToBeResubmitted[transactionUuid].raw_transaction) {
        ...
        await resendRawTxsObj.perform().catch(function(err) {
      ...
      lockReleasePromisesArray.push(oThis._releaseLock([transactionId]));
    }
    await Promise.all(lockReleasePromisesArray);
  },
  ...
  _resubmitGethNotFoundTxs: async function() {
    ...
    for (let clientId in oThis.clientRelatedMap) {
      ...
      while (transactionUuidsToBeResubmitted.length) {
        ...
        console.log('Transaction UUIDS Batch => ', transactionUuidsToBeResubmitted.length);

        // Fetch all the transactionLog information of a particular client.
        // Get transactionLogModel object of the specific client.
        console.log('Transaction log Before => ', transactionUuidsInBatch);
