  ...
  _fetchFromTransactionLog: async function() {
    ...
    for (let clientId in oThis.clientRelatedMapping) {
      ...
      while (transactionUuidsToBeResubmitted.length) {
        ...
        let transactionLogResponse = await new oThis.clientRelatedMapping[clientId].transactionLogModel({
          client_id: clientId,
          shard_name: oThis.clientRelatedMapping[clientId].shardName
        }).batchGetItem(transactionUuidsInBatch);

        oThis.transactionFetchResponse.push(transactionLogResponse);
      }
    }
  },
