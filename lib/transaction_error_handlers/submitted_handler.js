...
const SubmittedHandlerKlassPrototype = {
  ...
  asyncPerform: async function() {
    ...
    if (oThis.transactionsWithSubmittedStuck.length > 0) {
      ...
      await oThis._releaseLock(oThis.transactionsWithSubmittedStuck);
    }
  ...
  _resubmitTransaction: async function() {
    ...
    for (let transactionUuid in transactionsToBeResubmitted) {
      if (transactionsToBeResubmitted.hasOwnProperty(transactionUuid)) {
        ...
        let rawTx = JSON.parse(transactionUuid.raw_transaction),
        ...
        // Release lock.
        promisesArray.push(oThis._releaseLock([transactionUuid.transaction_hash]));
      }
    }
   ...
   * @private
   */
  _releaseLock: async function(transactionHashesToBeReleased) {
    const oThis = this;
