...
const SubmittedHandlerKlassPrototype = {
      ...
      if (transactionsToBeResubmitted[transactionUuid].raw_transaction) {
        ...
        let promise = new Promise(function(onResolve, onReject) {
          ...
          resendRawTxsObj
            .perform()
            .then(function() {
              oThis._releaseLock([transactionId]).then(function() {
                onResolve();
              });
            })
            .catch(function(err) {
        ...
        transactionSubmitPromises.push(promise);
      }
    }
    return Promise.all(transactionSubmitPromises);
  },
  ...
  _releaseLock: function(transactionIdsToBeReleased) {
    ...
    return new TransactionMetaModel().releaseLock(oThis.lockId, whereOptions, updateOptions);
  }
};
