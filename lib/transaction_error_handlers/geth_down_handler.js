...
const GethDownHandlerKlassPrototype = {
  ...
  asyncPerform: async function() {
    ...
    for (let index = 0; index < oThis.txMetaRows.length; index++) {
      ...
      if (
        txMetaRow.status != transactionMetaConstants.invertedStatuses[transactionMetaConstants.geth_down] &&
        txMetaRow.status != transactionMetaConstants.invertedStatuses[transactionMetaConstants.geth_out_of_sync]
      ) {
        anyRejections = true;
      }
    }
      ...
      for (let i = 0; i < txUuids.length; i++) {
        ...
        let promise = new Promise(function(onResolve, onReject) {
          new transferBtKlass(txParams)
            .perform()
            .then(function() {
              oThis._releaseTransactionLock(txUuid, txUuidToLockIdMap[txUuid]).then(function() {
                onResolve();
              });
            })
            .catch(function() {
