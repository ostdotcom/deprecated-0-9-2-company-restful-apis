...
const GethDownHandlerKlassPrototype = {
  ...
  asyncPerform: async function() {
    ...
    for (let index = 0; index < oThis.txMetaRows.length; index++) {
      ...
      if (
        txMetaRow.status != transactionMetaConstants.invertedStatuses[transactionMetaConsts.geth_down] &&
        txMetaRow.status != transactionMetaConstants.invertedStatuses[transactionMetaConsts.geth_out_of_sync]
      ) {
        anyRejections = true;
      }
    }
