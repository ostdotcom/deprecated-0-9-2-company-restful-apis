'use strict';
/**
 * To de-associate process from client in client_worker_managed_address_id
 *
...
DeAssociateWorker.prototype = {
  ...
  _sendCommandMsg: async function() {
    ...
    for (let i = 0; i < idsArray.length; i++) {
      ...
      // Publish the message.
      let commandThroughRMQ = await oThis.openStNotification.publishEvent.perform({
        topics: [queueTopic],
        publisher: 'OST_1',
        message: message
      });
    ...
    for (let i = 0; i < processDetails.length; i++) {
      ...
      await oThis.openStNotification.publishEvent.perform({
        topics: [topicName],
        publisher: 'OST_1',
        message: {
          kind: rmqQueueConstant.commandMsg,
