...
PopulateExchangeTradeDataKlass.prototype = {
  ...
  asyncPerform: async function() {
    ...
    await oThis.populateDataFromBitrex();
  ...
  populateDataFromBitrex: async function() {
    const oThis = this,
    ...
    for (let tradingPair in tradingPairTradingSymbolMap) {
      ...
      while (true) {
        logger.step('Starting to Fetch Data from Bitrex for: ', tradingSymbol, timestamp);
        ...
        for (let index = 0; index < recentTradesFromApi.length; index++) {
          ...
          hashToBeInserted['exchange'] =
            ExchangeTradeDataConstants.invertedExchanges[ExchangeTradeDataConstants.bitrexExchange];
