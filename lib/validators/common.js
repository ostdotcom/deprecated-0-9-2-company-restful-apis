...
CommonValidator.prototype = {
  ...
  validateArbitraryCommissionPercent(commission_percent, arbitrary_commission) {
    ...
    if (oThis.isVarTrue(arbitrary_commission) && commission_percent) {
      return false;
    }

    if (oThis.isVarFalse(arbitrary_commission) && !commission_percent) {
      return false;
    }
    return true;
  },
  ...
  validateArbitraryAmount: function (amount, arbitrary_amount) {
    ...
    if (oThis.isVarTrue(arbitrary_amount) && amount) {
      return false;
    }

    if (oThis.isVarFalse(arbitrary_amount) && !amount) {
      return false;
    }
  ...
  validateUsdAmount: function (amount) {
    if (amount && (amount < 0.001 || amount > 100)) {
      return false;
    }
    return true;
  },
