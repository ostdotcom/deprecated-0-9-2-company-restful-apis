 ...
 *
 ...
 var queryResponse = queryBuilder().update(['name=?, id=?', 'ACMA', 10], {touch: false}).update(['name=?, id=?', '1', 1000]).update({name: 'CMA', id: 0}).update({name: 'MA', id: 110}).update('id=30').update('id=40').where(['id IN (?) AND created_at > ?', [1,2], '2018-02-19']).where(['symbol_icon IN (?) AND symbol IS ?', ['ST1', 'ST2'], null]).where('symbol IS NULL').where('name="ST1"').where({conversion_rate: 3, symbol: 'ST1'}).where({conversion_rate: 2}).order_by('id ASC').order_by('client_id DESC').order_by(['name', 'reserve_managed_address_id']).order_by(['symbol']).order_by({id: 'asc', client_id: 'desc'}).order_by({symbol: 'AsC'}).limit(10).generate();
 var finalQuery = printQuery(queryResponse.data.query, queryResponse.data.queryData);
 finalQuery == "UPDATE `client_branded_tokens` SET name='ACMA', id=10, name='1', id=1000, `name`='CMA', `id`=0, `name`='MA', `id`=110, id=30, id=40 WHERE (id IN (1, 2) AND created_at > '2018-02-19') AND (symbol_icon IN ('ST1', 'ST2') AND symbol IS NULL) AND (symbol IS NULL) AND (name=\"ST1\") AND (`conversion_rate`=3 AND `symbol`='ST1') AND (`conversion_rate`=2) ORDER BY id ASC, client_id DESC, `name`, `reserve_managed_address_id`, `symbol`, `id` ASC, `client_id` DESC, `symbol` ASC LIMIT 10";
