 ...
 * calls airdrop pay to settle balances and updates transaction logs table.
 *
 */
...
const BlockScannerKlass = function(params) {
  ...
  oThis.lockId = Math.floor(new Date().getTime() + randomNumber);
...
BlockScannerKlass.prototype = {
  ...
  _getTxReceiptsForBatch: async function(batchedTxHashes, batchNo) {
    ...
    return new Promise(function(onResolve, onReject) {
      ...
      console.log('batchedTxHashes', batchedTxHashes);
      ...
      const requestCallback = function(err, result) {
        ...
        console.log('txHash, ', result.transactionHash, result);
        console.log('count', count, totalCount);

        oThis.txHashToTxReceiptMap[result.transactionHash] = result;
        console.log('===Key count receipt map 1', Object.keys(oThis.txHashToTxReceiptMap).length);
        count++;

        if (count === totalCount) {
          console.log('ResolveDcount', count, totalCount);
          onResolve();
        }
      };
    ...
    for (let clientId in oThis.recognizedTxUuidsGroupedByClientId) {
      ...
      while (true) {
        ...
        for (let txUuidIndex = 0; txUuidIndex < batchedTxUuids.length; txUuidIndex++) {
          ...
          console.log('===Key count receipt map 2', Object.keys(oThis.txHashToTxReceiptMap).length);
          console.log('====txHash', txHash, '===pushed at', Date.now());
          console.log('=====txHashToTxReceiptMap keys', Object.keys(oThis.txHashToTxReceiptMap), '==at', Date.now());
  ...
  collectDecodedEvents: async function() {
    ...
    console.log('====oThis.txHashToTxReceiptMap keys', Object.keys(oThis.txHashToTxReceiptMap), '==at', Date.now());
    console.log('===Key count receipt map 3', Object.keys(oThis.txHashToTxReceiptMap).length);
    ...
    for (let clientId in oThis.recognizedTxUuidsGroupedByClientId) {
      ...
      for (let txUuidsInd = 0; txUuidsInd < txUuids.length; txUuidsInd++) {
        ...
        if (!txReceipt) {
          console.log('txReceiptMissing', txHash, oThis.txHashToTxReceiptMap[txHash], oThis.txHashToTxReceiptMap);
        }
  ...
  generateToUpdateDataForUnKnownTx: async function() {
    ...
    for (let i = 0; i < oThis.unRecognizedTxHashes.length; i++) {
      let txHash = oThis.unRecognizedTxHashes[i],
        txReceipt = oThis.txHashToTxReceiptMap[txHash];
      console.log('===Key count receipt map 4', Object.keys(oThis.txHashToTxReceiptMap).length);
    ...
    for (let i = 0; i < oThis.unRecognizedTxHashes.length; i++) {
      let txHash = oThis.unRecognizedTxHashes[i];
      let txReceipt = oThis.txHashToTxReceiptMap[txHash];
      console.log('===Key count receipt map 5', Object.keys(oThis.txHashToTxReceiptMap).length);
    ...
    console.log('===Key count receipt map 6', Object.keys(oThis.txHashToTxReceiptMap).length);
    let params = {
