...
/**
 * This is class for block scanner which processes transactions for given block.
 *
 * Its main functionality is to categorize the transaction hashes (known and unknown), get the receipt
 * and collects decoded events, calls airdrop pay and updates transaction logs.
 *
 */
...
const logger = require(rootPrefix + '/lib/logger/custom_console_logger'),
  ...
  logsFolderAbsolutePath = homeAbsolutePath + '/openst-setup/logs';
 ...
 * @param params
 *
 * @param params.blockNumber = blockNumber for which transactions to be processed
 * @param params.provider = geth provider endpoint
 * @param params.transactionHashes = transaction hashes in current block obtained from transaction delegator
 * @param params.timeStamp = timeStamp obtained in blockDetails of current block
 * @param params.benchmarkFilePath = benchmarkFilePath
 * @constructor
 */
...
const BlockScannerKlass = function(params) {
  ...
  oThis.currentBlock = params.blockNumber;
...
BlockScannerKlass.prototype = {
  ...
  warmUpWeb3Pool: async function() {
    ...
    for (let i = 0; i < web3PoolSize; i++) {
      web3InteractFactory.getInstance('utility'); //TODO: Align it with read/write and master process
    }
  },
    ...
    const processNewBlocksAsync = async function() {
      try {
        ...
        if (oThis.interruptSignalObtained) {
          ...
          oThis.processFallouts(); //TODO: method to be filled with logic
        }
      }
    };
  ...
  _getTxReceiptsForBatch: async function(batchedTxHashes) {
    const oThis = this,
  ...
  getTransactionReceipts: async function() {
    ...
    let batchNo = 1,
      ...
      loadPerConnection = parseInt(oThis.transactionHashes.length / web3PoolSize) + 1,
      promiseArray = [];

    loadPerConnection = 30;

    if (loadPerConnection < 5) loadPerConnection = oThis.transactionHashes.length;

    while (true) {
      const offset = (batchNo - 1) * loadPerConnection,
        batchedTxHashes = oThis.transactionHashes.slice(offset, loadPerConnection + offset);

      batchNo = batchNo + 1;

      if (batchedTxHashes.length === 0) break;

      promiseArray.push(oThis._getTxReceiptsForBatch(batchedTxHashes));
    }
   ...
   * Process fallouts
   */
  processFallouts: function() {
    const oThis = this;

    // TODO: Need an email for any fallouts with transaction hashes unprocessed

    console.error('=====There are fallouts in Block: ', oThis.currentBlock);
  },
