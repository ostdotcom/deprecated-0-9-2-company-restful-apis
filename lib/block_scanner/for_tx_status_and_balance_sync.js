...
let abiDecoderInitiated = false,
  // init a global var which would be used across all instances to keep lock id
  currentLockIdPrefix = 1;
...
const BlockScannerKlass = function(params) {
  ...
  oThis.lockIdPrefix = currentLockIdPrefix;
  // increase lock prefix for next iteration
  currentLockIdPrefix = currentLockIdPrefix + 1;
...
BlockScannerKlass.prototype = {
  ...
  _getLockId: function() {
    const oThis = this;
    return parseFloat(oThis.lockIdPrefix + '.' + oThis.processId);
  },
