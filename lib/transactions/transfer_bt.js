...
TransferBt.prototype = {
  ...
  _fetchTransactionLog: async function() {
    ...
    // check if the transaction log uuid is same as that passed in the params, otherwise error out
    if (!transactionLog) {
      logger.notify('l_t_tb_1', 'Transaction uuid ' + oThis.transactionUuid + ' is not present in ddb');
  ...
  updateParentTransactionLog: function(statusString, failedResponse) {
    ...
    if (oThis.rawTransactionDetails) {
      dataToUpdate['nonce'] = oThis.rawTransactionDetails.nonce;
      dataToUpdate['transaction_executor_address'] = oThis.rawTransactionDetails.from;
      dataToUpdate['raw_transaction'] = JSON.stringify(oThis.rawTransactionDetails);
    }
  ...
  sendAirdropPay: async function() {
      ...
      if (moUtils.isChainNodeDownError(error)) {
        return Promise.resolve(
          responseHelper.error({
            ...
            api_error_identifier: 'chain_node_down',
            debug_options: {}
          })
        );
      } else {
    ...
    // update transaction meta table
    await new TransactionMetaModel()
      .update([
        'transaction_hash = ?,post_receipt_process_params = ?,status = ?',
        oThis.transactionHash,
        JSON.stringify(payResponse.data.post_receipt_process_params),
        transactionMetaConstants.invertedStatuses[transactionMetaConstants.submitted]
      ])
