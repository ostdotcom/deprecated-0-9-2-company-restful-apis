...
TransferBt.prototype = {
  ...
  updateParentTransactionLog: async function(params) {
    ...
    if (response.isSuccess()) {
      ...
      if (dataToUpdateInTxLog['error_code'] === recognizedInternalErrorIdentifiers.chainNodeDown) {
      ...
      } else if (dataToUpdateInTxLog['error_code'] === recognizedInternalErrorIdentifiers.gasLowError) {
      ...
      } else if (dataToUpdateInTxLog['error_code'] === recognizedInternalErrorIdentifiers.replacementTxUnderPriced) {
        txMetaStatusString = transactionMetaConstants.replacement_tx_under_priced;
        txLogStatus = transactionLogConst.invertedStatuses[transactionLogConst.failedStatus];
      } else if (dataToUpdateInTxLog['error_code'] === recognizedInternalErrorIdentifiers.nonceTooLow) {
