...
ApproveContractKlass.prototype = {
  ...
  _fetchUserAddress: async function() {
    ...
    const managedAddressCacheFetchResponse = await new ManagedAddressCacheKlass({
      uuids: [oThis.approverUuid]
    }).fetch();

    if (managedAddressCacheFetchResponse.isFailure()) {
      logger.error(managedAddressCacheFetchResponse.toHash());
      managedAddressCacheFetchResponse.data = Object.assign({
        input_params: oThis.approveBTInput,
        error: 'Failed while approving contract. ' + JSON.stringify(managedAddressCacheFetchResponse.toHash())
      });
  ...
  _approve: async function() {
    ...
    const approveResponse = await new openSTPlaform.services.approve.brandedToken(oThis.approveBTInput).perform();

    if (approveResponse.isFailure()) {
      approveResponse.data = Object.assign({
        input_params: oThis.approveBTInput,
        error: 'Failed while approving contract. ' + JSON.stringify(approveResponse.toHash())
      });
    ...
    logger.debug(
      'approve info:',
      'approve tx hash:',
      approveResponse.data.rawTransactionReceipt.transactionHash,
      'from addr:',
      approveResponse.data.rawTransactionReceipt.from,
      'status:',
      approveResponse.data.rawTransactionReceipt.status
    );
  ...
  _markBtContractApprovedProperty: function() {
    ...
    const bTContractApprovedIntProp = new ManagedAddressModel().invertedProperties[
      managedAddressesConst.bTContractApproved
    ];

    new ManagedAddressModel()
      .update(['properties = properties | ?', bTContractApprovedIntProp])
...
InstanceComposer.registerShadowableClass(ApproveContractKlass, 'getApproveContractClass');
