...
const ApproveContractKlass = function(params){
  ...
  oThis.approveAmount = basicHelper.convertToWei('1000000000');
  ...
  //   approver_address: params.approver_address,
  //   approver_passphrase: params.approver_passphrase,
  //   approvee_address: params.approvee_address,
  //   to_approve_amount: oThis.approveAmount,
  //   options: {returnType: params.returnType || 'txReceipt'}
  // }
...
ApproveContractKlass.prototype = {
  ...
  perform: async function(){
    ...
    var addressCachedObj = new ManagedAddressCacheKlass({'uuids': [oThis.approverUuid]})
      , approverAddressResp = await addressCachedObj.fetchDecryptedData(["private_key"])
      , approveResponse = {};

    if(approverAddressResp.isFailure()){
      approveResponse = responseHelper.error("l_t_ac_1", approverAddressResp.err);
      approveResponse[data] = Object.assign({input_params: approveBTInput, error: "Failed while approving contract. " + approverAddressResp.err});
      return Promise.resolve(approveResponse);
    }

    var userAddress = approverAddressResp[oThis.approverUuid];
