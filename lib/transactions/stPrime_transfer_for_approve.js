...
TransferSTPrimeForApproveKlass.prototype = {
  ...
  perform: async function(){
    ...
    if(addressResponse.isFailure()){
      addressResponse['data'] = Object.assign({error: "Failed while transferring st prime."}, addressResponse.toHash().err);
    ...
    if(!estimateGasResponse || estimateGasResponse.isFailure()){
      estimateGasResponse.data = Object.assign({input_params: estimateGasInput, error: "Failed while transferring st prime."}, estimateGasResponse.toHash().err);
    ...
    if (transferResponse.isFailure()) {
      transferResponse['data'] = Object.assign({input_params: transferSTPrimeInput, error: "Failed while Transfering ST Prime. " + transferResponse.toHash().err});
    ...
    while(maxBalanceFetch--) {
      ...
      if(fetchedBalance.gte(estimatedGasWei)){
        ...
        logger.debug('ST PRIME BALANCE:: less than the approve estimate gas for address:', receiverAddress.ethereum_address);
      }
    }
