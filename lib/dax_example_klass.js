...
const daxExampleKlass = function() {
  ...
  oThis.daxClient = new AmazonDaxClient({
    endpoints: ['kit-saas-siege.xsap5a.clustercfg.dax.use1.cache.amazonaws.com:8111'],
    //endpoints: ['kit-saasapi-staging.xsap5a.clustercfg.dax.use1.cache.amazonaws.com:8111'],
    apiVersion: '2017-04-19',
    region: region,
    accessKeyId: 'AKIAIRGX5FR43AEDHUFQ',
    secretAccessKey: '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',
    writeRetries: 10,
    readRetries: 10
  });
...
daxExampleKlass.prototype = {
  ...
  getItemDax: async function(totalGets) {
    ...
    for (var uuid in oThis.createdUuidsInDynamo) {
      ...
      await (async function() {
        return new Promise(function(onResolve, onReject) {
          oThis.daxClient.getItem(params, function(err, data) {
            if (err) {
    ...
    console.log(
      '\t DAX ----- Total Get Requested - ',
      totalGets,
      '\tTotal time: ',
      endTime - startTime,
      'ms - Avg time: ',
      (endTime - startTime) / totalGets,
      'ms'
    );
  },
  ...
  batchGetItemDax: async function(totalGets, batchSize) {
    ...
    console.log(
      ...
      endTime - startTime,
      'ms - Avg time: ',
      ((endTime - startTime) * batchSize) / totalGets,
      'ms'
    );
  },
  ...
  putItemDax: async function(totalWrites) {
    ...
    for (var uuid in oThis.generatedUuids) {
      ...
      var daxClient = new AmazonDaxClient({
        endpoints: ['kit-saas-siege.xsap5a.clustercfg.dax.use1.cache.amazonaws.com:8111'],
        //endpoints: ['kit-saasapi-staging.xsap5a.clustercfg.dax.use1.cache.amazonaws.com:8111'],
        apiVersion: '2017-04-19',
        region: region,
      ...
      (async function() {
        return new Promise(function(onResolve, onReject) {
          daxClient.putItem(params, function(err, data) {
            if (err) {
              console.error('Unable to read item. Error JSON:', JSON.stringify(err, null, 2));
            } else {
    ...
    var endTime = new Date().getTime();
    console.log(
      '\t DAX ----- Total Writes Requested : ',
      totalWrites,
      '\tTotal time: ',
      endTime - startTime,
      'ms - Avg time: ',
      (endTime - startTime) / totalWrites,
      'ms'
    );
  },
  ...
  updateItemDax: async function(totalWrites) {
    ...
    for (var uuid in oThis.createdUuidsInDynamo) {
      ...
      await (async function() {
        return new Promise(function(onResolve, onReject) {
          oThis.daxClient.updateItem(params, function(err, data) {
            if (err) {
              console.error('Unable to read item. Error JSON:', JSON.stringify(err, null, 2));
            } else {
              oThis.createdUuidsInDynamo[uuid] = oThis.generatedUuids[uuid];
