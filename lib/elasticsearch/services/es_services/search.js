...
Service.prototype = {
    ...
    try {
      ...
      return client.search( oThis.buildParams() )
        .then( function ( clientResponse ) {
          logger.win("search Operation Successful!");
  ...
  , formatSearchResponse: function ( params, response ) {
    ...
    //-- if paramsFrom = 10
    //-- last search result index = 10 + 10 = 20
    // 24 > 20 = true (we have next page)

    //-- if paramsFrom = 14
    //-- last search result index = 10 + 14 = 24
    // 24 > 24 = false (we do not have next page).

    //-- if paramsFrom = 13
    //-- last search result index = 10 + 13 = 23
    // 24 > 23 = true (we have next page with 1 result)

    if ( (total - 1) > (paramsSize + paramsFrom ) ) {
      // We have next page.
