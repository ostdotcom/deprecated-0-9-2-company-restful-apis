...
try {
  ...
  logger.error('esConstants', esConstants);
  logger.error('esConstants_ES_HOST', esConstants.ES_HOST);
...
if (esConstants.ENVIRONMENT != 'development') {
  ...
  elasticSearchClientConfig.awsConfig = new aws.Config({
    credentials: new aws.Credentials(esConstants.AWS_ES_ACCESS_KEY, esConstants.AWS_ES_SECRET_KEY),
    region: esConstants.AWS_ES_REGION
  });
}
