...
let basicRecord = {
  ...
  eventSourceARN:
    'arn:aws:dynamodb:us-east-1:704700004548:table/s_sb_transaction_logs_shard_004/stream/2018-06-05T16:17:14.352'
};
let noOfRecords = 20;
...
let createRecords = function() {
  ...
  lambda.handler(eventData, null, function(errResponse, successResponse) {
    ...
    logger.step(
      '--------- Sleeping for 10 seconds (Waiting for shards to make create operation be visible to search) ------'
    );
...
let searchRecords = function(query, source) {
  ...
  return manifest.services.transactionLog
    .search(query, source)
