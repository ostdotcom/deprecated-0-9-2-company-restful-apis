...
ProposeKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_ob_p_1", "Inside catch block", {error: error}, {sendErrorEmail: true});
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    const oThis = this
        , criticalChainInteractionLogs = await new CriticalChainInteractionLogModel().getByIds([oThis.criticalChainInteractionLogId])
    ...
    if (!criticalChainInteractionLog) {
      const errorRsp = responseHelper.error("l_snm_p_2", "criticalChainInteractionLog not found", {}, {sendErrorEmail: false});
  ...
  setDerivedParams: async function () {
    ...
    if(!clientBrandedToken){
      return Promise.reject(responseHelper.error('l_snm_p_3', 'no client branded token found for id'+oThis.criticalChainInteractionLog.client_branded_token_id,
