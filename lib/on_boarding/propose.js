...
ProposeKlass.prototype = {
  ...
  asyncPerform: async function() {
    ...
    new Erc20ContractAddressCacheKlass({
      addresses: [registerationData.erc20_address],
      chain_id: configStrategy.OST_UTILITY_CHAIN_ID
    }).clear();

    new Erc20ContractUuidCacheKlass({
      uuids: [registerationData.uuid],
      chain_id: configStrategy.OST_UTILITY_CHAIN_ID
    }).clear();
   ...
   *
   * @return {Promise<result>} - returns an object of Result
   *
   */
    ...
    return new Promise(function(onResolve, onReject) {
      ...
      const getStatus = async function() {
        if (maxAttempts > 0) {
          ...
          getRegistrationStatusResponse = await getRegistrationStatusServiceObj.perform();
