...
SetAcceptedMarginKlass.prototype = {
  ...
  asyncPerform: async function () {
    ...
    if(setAcceptedMarginResponse.isFailure()){
      return Promise.reject(responseHelper.error('l_ob_spo_2', 'setAcceptedMargin failed. details:\n', setAcceptedMarginResponse));
    } else {
      await new CriticalChainInteractionLogModel().updateCriticalChainInteractionLog(
        oThis.criticalChainInteractionLogId,
        {
          status: new CriticalChainInteractionLogModel().invertedStatuses[criticalChainInteractionLogConst.processedStatus],
          transaction_hash: setAcceptedMarginResponse.data.rawTransactionReceipt.transactionHash
        },
        oThis.parentCriticalInteractionLogId,
        oThis.clientTokenId
      );
