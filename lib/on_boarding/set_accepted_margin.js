...
SetAcceptedMarginKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_ob_sam_1", "Inside catch block", null, {error: error}, {sendErrorEmail: true});
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_sam_2", "criticalChainInteractionLog not found", null, {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_sam_2", "parentCriticalChainInteractionLog not found", null, {},
        {sendErrorEmail: false}));
    }
