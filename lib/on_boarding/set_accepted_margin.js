...
SetAcceptedMarginKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_ob_sam_1", "Inside catch block", {error: error}, {sendErrorEmail: true, clientId: oThis.clientId});
  ...
  asyncPerform: async function () {
    ...
    if(setAcceptedMarginResponse.isFailure()){
      return Promise.reject(responseHelper.error('l_ob_sam_2', 'setAcceptedMargin failed. details:\n', setAcceptedMarginResponse));
    } else {
      await new CriticalChainInteractionLogModel().updateCriticalChainInteractionLog(
        oThis.criticalChainInteractionLogId,
        {
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_sam_2", "criticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_sam_2", "parentCriticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }
  ...
  validateAndSanitize: async function () {
    ...
    oThis._criticalLogDebug('* Valdating set accepted margin params', 'debug');
