...
SetWorkersKlass.prototype = {
    ...
    return oThis.asyncPerform()
        .catch(function (error) {
          ...
          if(responseHelper.isCustomResult(error)) {
            ...
            errorObj = responseHelper.error("l_sw_1", "Inside catch block", {error: error}, {sendErrorEmail: true, clientId: oThis.clientId});
  ...
  asyncPerform: async function () {
    ...
    for(var i=0; i<promiseResolvers.length; i++) {
      ...
      if (r.isFailure()) {
        logger.notify('l_sw_2', 'Set Worker Failed', r.toHash(), {clientId: oThis.clientId});
      } else {
    ...
    if (successWorkerAddrIds.length == 0) {
      const errorRsp = responseHelper.error(
          ...
          {data: formattedPromiseResponses}
      );
  ...
  validateAndSanitize: async function () {
    ...
    const existingWorkerManagedAddresses = await new ClientWorkerManagedAddressIdModel().getInActiveByClientId(oThis.clientId);
