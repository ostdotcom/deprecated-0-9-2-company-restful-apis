...
SetWorkersKlass.prototype = {
  ...
  asyncPerform: async function() {
    ...
    let workerAddrs = Object.keys(oThis.workerAddressesIdMap),
      promiseResolvers = [],
    ...
    let promiseResponses = await Promise.all(promiseResolvers);

    for (let i = 0; i < promiseResolvers.length; i++) {
      let r = promiseResponses[i];
      if (r.isFailure()) {
        notifier.notify('l_sw_2', 'Set Worker Failed', r, { clientId: oThis.clientId });
      } else {
    ...
    const invertedStatus = new ProcessQueueAssociationModel().invertedStatuses[
      processQueueAssocConst.availableForAllocations
    ];

    const availableProcesses = await new ProcessQueueAssociationModel()
      .select('*')
      .where(['status=? AND chain_id=?', invertedStatus, oThis.chainId])
      .fire();

    // Get minimum of two to decide looping max.
    let minOfTwoArrays = Math.min(successWorkerAddrIds.length, availableProcesses.length);

    // Associate processes to client workerIds.
    for (let index = 0; index < minOfTwoArrays; index++) {
      let updateParams = {
        id: successWorkerAddrIds[index],
        process_id: availableProcesses[index].process_id
      };
  ...
  validateAndSanitize: async function() {
    ...
    oThis.chainId = brandedToken.chain_id;
    ...
    let managedAddressIdClientWorkerAddrIdMap = {};
