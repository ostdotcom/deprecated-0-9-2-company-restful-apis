...
SetWorkersKlass.prototype = {
  ...
  asyncPerform: async function() {
    ...
    const availableProcesses = await new ProcessQueueAssociationModel().getProcessesByStatus(
      processQueueAssocConst.availableForAllocations
    );

    // Associate processes to client workerIds.
    if (successWorkerAddrIds.length === availableProcesses.length) {
      for (let index = 0; index < availableProcesses.length; index++) {
        let updateParams = {
      ...
      for (let index = 0; index < minOfTwoArrays.length; index++) {
        let updateParams = {
          id: successWorkerAddrIds[index],
          process_id: availableProcesses[index].process_id
        };
