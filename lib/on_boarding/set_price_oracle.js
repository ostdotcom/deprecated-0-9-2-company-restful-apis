...
SetPriceOracleKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_ob_spo_1", "Inside catch block", null, {error: error}, {sendErrorEmail: true});
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_spo_3", "criticalChainInteractionLog not found", null, {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_spo_4", "parentCriticalChainInteractionLog not found", null, {},
        {sendErrorEmail: false}));
    }
  ...
  validateAndSanitize: async function () {
    ...
    if (!oThis.brandedTokenId || !oThis.clientId) {
      return Promise.reject(responseHelper.error('l_ob_spo_5', 'Mandatory params missing.', null, {},
        {sendErrorEmail: false}));
    }
    ...
    if (oThis.brandedToken.client_id != oThis.clientId) {
      return Promise.reject(responseHelper.error('l_ob_spo_6', 'Unauthorised request'));
    }

    oThis.airDropContractAddress = oThis.brandedToken.airdrop_contract_addr;

    if (!oThis.airDropContractAddress) {
      return Promise.reject(responseHelper.error('l_ob_spo_7', 'Airdrop contract address is mandatory.', null, {},
        {sendErrorEmail: false}));
    }
