...
SetPriceOracleKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function (error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_ob_spo_1", "Inside catch block", {error: error}, {sendErrorEmail: true, clientId: oThis.clientId});
        }
  ...
  setCriticalChainInteractionLog: async function () {
    ...
    if (!criticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_spo_3", "criticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }

    if (!parentCriticalChainInteractionLog) {
      return Promise.reject(responseHelper.error("l_ob_spo_4", "parentCriticalChainInteractionLog not found", {},
        {sendErrorEmail: false}));
    }
  ...
  validateAndSanitize: async function () {
    ...
    if (!oThis.brandedTokenId || !oThis.clientId) {
      return Promise.reject(responseHelper.error('l_ob_spo_5', 'Mandatory params missing.', {},
        {sendErrorEmail: false}));
    }
    ...
    if (!oThis.airDropContractAddress) {
      return Promise.reject(responseHelper.error('l_ob_spo_7', 'Airdrop contract address is mandatory.', {},
        {sendErrorEmail: false}));
    }
