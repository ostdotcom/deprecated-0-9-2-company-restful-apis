...
AssignStrategies.prototype = {
  ...
  asyncPerform: async function() {
    ...
    let clientConfigStrategyCacheObj = new clientConfigStrategyCacheKlass({ clientIds: [oThis.clientId] }),
    ...
    let cacheRsp = strategyIdsFetchRsp.data[oThis.clientId];

    let configStrategyData = [],
      existingStrategyIdsArray = [];
    if (cacheRsp && cacheRsp['configStrategyIds'].length > 0) {
      existingStrategyIdsArray = cacheRsp['configStrategyIds'];
    ...
    for (let i = 0; i < missingStrategyKinds.length; i++) {
      ...
      if (missingStrategyKinds[i] === configStrategyConstants.dynamo) {
   ...
   * @param {number}kind: kind for which some strategy id is needed.
   * @return {number}: strategy id for the kind
   */
  getStrategyIdForKind: async function(kind) {
    const oThis = this;

    let strategyIdsForSpecificKind = await new ConfigStrategyModel()._getStrategyIdsByKindAndGroupId(
      kind,
      oThis.groupId
    );
