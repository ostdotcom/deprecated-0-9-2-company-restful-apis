...
AllocateAirdropRouterKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(function(error) {
        if (responseHelper.isCustomResult(error)){
          ...
          return responseHelper.error("l_aa_r_1", "Unhandled result", {}, {});
        }
      });
  },
  ...
  asyncPerform: function () {
    ...
    return Promise.reject(responseHelper.error("l_aa_r_2", "Unhandled current step.", {}, {sendErrorEmail: false}));
  },
    ...
    if(oThis.parentCriticalInteractionLogId) {
      ...
      if(similarRecordIds.length > 1) {
        if (Math.min(...similarRecordIds) != dbRecord.insertId) {
          ...
          return Promise.resolve(responseHelper.error("l_aa_r_4", "duplicate record found. self destruct called.", {},
            {sendErrorEmail: false}));
        }
      }
    }
