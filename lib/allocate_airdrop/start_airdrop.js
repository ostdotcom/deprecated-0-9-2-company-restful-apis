 ...
 *
 * @module lib/allocate_airdrop/start_airdrop
 *
 */
...
StartAllocateAirdropKlass.prototype = {
  ...
  validateIncompleteRequests: async function() {
    ...
    if (clientAirdrops.length > 0) {
      for (let i = 0; i < clientAirdrops.length; i++) {
        if (
  ...
  validateReserveBalance: async function() {
    ...
    const brandedTokenBalanceResponse = await new openSTPlaform.services.balance.brandedToken({
  ...
  insertDb: async function() {
    ...
    let airdrop_list_type = new ClientAirdropModel().invertedAirdropListType[
    ...
    await openSTNotification.publishEvent
      ...
      .catch(function(err) {
        logger.error('Message for starting airdrop was not published. Payload: ', payload, ' Error: ', err);
      });
  }
};

InstanceComposer.registerShadowableClass(StartAllocateAirdropKlass, 'getStartAllocateAirdropClass');
