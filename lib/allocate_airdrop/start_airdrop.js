...
const StartAllocateAirdropKlass = function (params) {
  ...
  oThis.parentCriticalInteractionLogId = parseInt(params.parent_critical_interaction_log_id) || oThis.criticalChainInteractionLogId;
...
StartAllocateAirdropKlass.prototype = {
  ...
  validateInput: async function () {
    const oThis = this,
      ...
      BTCacheKlass = oThis.ic().getClientBrandedTokenCacheKlass();
    ...
    if (![clientAirdropConst.allAddressesAirdropListType,
        ...
        clientAirdropConst.everAirdroppedAddressesAirdropListType].includes(oThis.airdropParams.airdrop_user_list_type)) {
  ...
  validateUserIds: async function(){
    ...
    if(oThis.airdropParams.user_ids){
      ...
      const uuidsAddressInfo = await new ManagedAddressModel().select('*')
        .where(['uuid in (?)', oThis.airdropParams.user_ids]).fire();
  ...
  validateIncompleteRequests: async function () {
    ...
    if (clientAirdrops.length > 0) {
      for (var i = 0; i < clientAirdrops.length; i++) {
        if ([clientAirdropConst.incompleteStatus, clientAirdropConst.processingStatus]
            .includes(new ClientAirdropModel().statuses[clientAirdrops[i].status])) {
  ...
  validateReserveBalance: async function () {
    ...
    const brandedTokenBalanceResponse = await openStPlatform.services.balance.brandedToken(
      {
    ...
    } else if (oThis.airdropParams.airdrop_user_list_type == clientAirdropConst.everAirdroppedAddressesAirdropListType){
      getFilteredActiveUsersCountParams['property_set_bit_value'] = new ManagedAddressModel()
        .invertedProperties[managedAddressesConst.airdropGrantProperty]
    }
    if(oThis.airdropParams.user_ids){
    ...
    console.log('-------------------------------oThis.airdropParams.airdrop_amount-', oThis.airdropParams.airdrop_amount);
    ...
    if (amountInWei.mul(getFilteredActiveUsersCountResponse[0].total_count).gt(basicHelper.convertToBigNumber(brandedTokenBalanceResponse.data.balance))) {
  ...
  insertDb: async function () {
    ...
    var airdrop_list_type = new ClientAirdropModel().invertedAirdropListType[oThis.airdropParams.airdrop_user_list_type];
    if(oThis.airdropParams.user_ids){
      airdrop_list_type |= new ClientAirdropModel().invertedAirdropListType[clientAirdropConst.specificAddressesAirdropListType];
    }

    const clientAirdropCreateResponse = await new ClientAirdropModel().insert({
