...
StartAllocateAirdropKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function(error) {
        ...
        if(responseHelper.isCustomResult(error)) {
          ...
          errorObj = responseHelper.error("l_aa_sa_1", "Unhandled result", {error: error}, {sendErrorEmail: true, clientId: oThis.clientId});
  ...
  validateInput: async function () {
    ...
    if (!oThis.airdropParams || !oThis.clientId) {
      return Promise.reject(responseHelper.error("l_aa_sa_3", "Required params missing", {},
        {sendErrorEmail: false}));
    }
    ...
    if (![clientAirdropConst.allAddressesAirdropListType,
        clientAirdropConst.neverAirdroppedAddressesAirdropListType].includes(oThis.airdropParams.airdrop_user_list_type)) {
    ...
    if (!oThis.tokenSymbol) {
      return Promise.reject(responseHelper.error("l_aa_sa_6", "Invalid Token Symbol", {}, {sendErrorEmail: false}));
    }
    ...
    if (oThis.clientId != cacheRsp.data.client_id) {
      return Promise.reject(responseHelper.error("l_aa_sa_7", "Invalid client id", {}, {sendErrorEmail: false}));
    }
    ...
    if (clientAirdrops.length > 0) {
      for (var i = 0; i < clientAirdrops.length; i++) {
        if ([clientAirdropConst.incompleteStatus, clientAirdropConst.processingStatus]
          ...
          return Promise.reject(responseHelper.error("l_aa_sa_8", "Airdrop requests are in-process", {},
            {sendErrorEmail: false}));
  ...
  validateReserveBalance: async function () {
    ...
    if (brandedTokenBalanceResponse.isFailure()) {
      return Promise.reject(responseHelper.error("l_aa_sa_9", "Something went wrong.", {},
        {sendErrorEmail: false}));
    }
  ...
  insertDb: async function () {
    ...
    const clientAirdropCreateResponse = await new ClientAirdropModel().insert({
      ...
      airdrop_list_type: new ClientAirdropModel().invertedAirdropListType[oThis.airdropParams.airdrop_user_list_type],
