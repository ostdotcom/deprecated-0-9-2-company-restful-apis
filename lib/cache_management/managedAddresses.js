...
const rootPrefix = '../..'
    ...
    , ManagedAddressSaltCacheKlass = require(rootPrefix + '/lib/cache_management/managed_address_salt')
...
managedAddressCache.prototype.fetchDecryptedData = async function(properties) {
  ...
  const managedAddressSaltCache = new ManagedAddressSaltCacheKlass({'client_id': dataFromCache.client_id});
  const saltCacheFetchRsp = await managedAddressSaltCache.fetch();

  if (saltCacheFetchRsp.isFailure()) {
    return saltCacheFetchRsp;
  }
  ...
  const responseData = util.clone(dataFromCache) // this is to prevent inmemory cache not modifiying the cache object with decrtpyed values
      , addrSaltEncr = saltCacheFetchRsp.data.addressSalt
      , addrSaltDecr = await localCipher.decrypt(coreConstants.CACHE_SHA_KEY, addrSaltEncr);

  if (properties.includes('ethereum_address')) {
    responseData['ethereum_address_d'] = await localCipher.decrypt(addrSaltDecr, responseData['ethereum_address_e']);
  }

  if (properties.includes('passphrase')) {
    responseData['passphrase_d'] = await localCipher.decrypt(addrSaltDecr, responseData['passphrase_e']);
  }
