...
UserStPrimeAvailability.prototype = {
    ...
    try {
      ...
      web3Provider.eth.getBalance(address, function (error, result) {
        if (error) {
          ...
          return Promise.onResolve(responseHelper.error('l_cm_usta_getSTPrimeBalanceFor_1', 'Error while getting the balance'));
        }
      ...
      return Promise.onResolve(responseHelper.error('l_cm_usta_getSTPrimeBalanceFor_2', 'Something went wrong'));
    }
  ...
  isSTPrimeBalanceAvailable: async function (address) {
    ...
    try {
      ...
      return Promise.onResolve(responseHelper.error('l_cm_usta_isSTPrimeBalanceAvailable_1', 'Something went wrong while getting from cache'));

    } catch (err) {
      //Format the error
      logger.error("app/services/user_stPrime_availability.js:isSTPrimeBalanceAvailable inside catch ", err);
      return Promise.onResolve(responseHelper.error('l_cm_usta_isSTPrimeBalanceAvailable_2', 'Something went wrong'));
    }
  ...
  _updateSTPrimeCache: async function (address, value) {
    ...
    try {
      ...
      return Promise.onResolve(responseHelper.error('l_cm_usta_updateSTPrimeCache_2', 'Something went wrong'));
    }
  }
