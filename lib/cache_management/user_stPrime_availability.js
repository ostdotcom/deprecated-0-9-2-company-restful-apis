...
UserStPrimeAvailability.prototype = {
  ...
  getSTPrimeBalanceFor: async function (address) {
    ...
    try {
      if (!basicHelper.isAddressValid(address)) {
        return Promise.resolve(responseHelper.error('a_s_u_getSTPrimeBalanceFor_1', "Invalid address",
  ...
  isSTPrimeBalanceAvailable: async function (address) {
    ...
    try {
      if (!basicHelper.isAddressValid(address)) {
        return Promise.resolve(responseHelper.error('l_cm_usta_isSTPrimeBalanceAvailable_1', "Invalid address",
          {}, {sendErrorEmail: false}));
      }
  ...
  _updateSTPrimeCache: async function (address, value) {
    ...
    try {
      if (!basicHelper.isAddressValid(address)) {
        return Promise.resolve(responseHelper.error('l_cm_usta_updateSTPrimeCache_1', "Invalid address",
          {}, {sendErrorEmail: false}));
      }
