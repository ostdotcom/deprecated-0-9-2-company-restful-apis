...
const ProcessQueueAssociationKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    const oThis = this,
      clientWorkerManagedAddressIdRsp = await new ClientWorkerManagedAddressIdModel().getActiveAndHoldByClientId(
        oThis.clientId
      );
    // The above method call will fetch processIds for all active workers.

    let finalRecords = { activeProcesses: [], holdProcesses: [] };

    if (clientWorkerManagedAddressIdRsp.length !== 0) {
      let processIds = [],
        managedAddressIds = [],
        processAndStatusMapping = {},
      ...
      for (let i = 0; i < clientWorkerManagedAddressIdRsp.length; i++) {
        ...
        processAndStatusMapping[clientWorkerManagedAddressIdRsp[i].process_id] =
          clientWorkerManagedAddressIdRsp[i].status;
      }
      ...
      let holdRecords = [],
        activeRecords = [],
        processDetailsMap = await new ProcessQueueAssociationModel().getByProcessIds(processIds);
      ...
      for (let processId in processDetailsMap) {
        ...
        if (processDetails.status !== processQueueAssocConst.processKilled) {
          let processRecord = {
            ...
            topic_name: processDetails.topic_name,
          ...
          // Active process
          if (
            processAndStatusMapping[processDetails.process_id] ===
            ...
            activeRecords.push(processRecord);
          }
          ...
          // Hold process
            ...
            holdRecords.push(processRecord);
          }
        }
      }
      finalRecords['activeProcesses'] = activeRecords;
      finalRecords['holdProcesses'] = holdRecords;
    }

    return Promise.resolve(responseHelper.successWithData({ processDetails: finalRecords }));
  }
};
