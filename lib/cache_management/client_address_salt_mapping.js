 ...
 *
 */
const ClientAddressSaltMappingCacheKlass = function(params) {
  const oThis = this;
...
ClientAddressSaltMappingCacheKlass.prototype = Object.create(baseCache.prototype);
...
const ClientAddressSaltMappingKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    let saltId = response.length > 0 ? response[0].id : 0;

    return Promise.resolve(responseHelper.successWithData({ clientAddrSalt: saltId }));
  }
};

Object.assign(ClientAddressSaltMappingCacheKlass.prototype, ClientAddressSaltMappingKlassPrototype);

InstanceComposer.registerShadowableClass(ClientAddressSaltMappingCacheKlass, 'getClientAddressSaltMappingCache');

module.exports = ClientAddressSaltMappingCacheKlass;
