...
clientTransactionTypeCache.prototype.fetchDataFromSource = async function() {
  ...
  var dbRecord = await new ClientTransactionTypeModel()
    .getTransactionByName({clientId: oThis.clientId, name: oThis.transactionKind});

  if(!dbRecord[0]) {
    return responseHelper.error("cm_ctt_1", "Invalid transaction type");
  }

  var res = dbRecord[0];
  var currency_value = null;
  if(res.currency_type == new ClientTransactionTypeModel().invertedCurrencyTypes[clientTxTypesConst.btCurrencyType]){
  ...
  var data = {
    id: res.id,
    name: res.name,
    kind: new ClientTransactionTypeModel().kinds[res.kind],
    currency_type: new ClientTransactionTypeModel().currencyTypes[res.currency_type],
    currency_value: currency_value,
    commission_percent: res.commission_percent,
    status: new ClientTransactionTypeModel().statuses[res.status]
  };
