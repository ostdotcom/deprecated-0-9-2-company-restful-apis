...
const OstPricePointsCacheKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    for(var key in (new CurrencyConversionRateModel().quote_currency)){
      var currencyCode = new CurrencyConversionRateModel().quote_currency[key];

      var response = await new CurrencyConversionRateModel().getLastActiveRates(currencyCode);
      if(response[0]) {
        hasData = true;
