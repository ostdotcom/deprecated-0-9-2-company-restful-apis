...
const OstPricePointsCacheKlassPrototype = {
  ...
  setCacheKey: function() {
    ...
    oThis.cacheKey = oThis._sharedCacheKeyPrefix() + 'c_ost_pp_' + configStrategy.OST_UTILITY_CHAIN_ID;
  ...
  fetchDataFromSource: async function() {
    ...
    for (let key in new CurrencyConversionRateModel().quote_currency) {
      ...
      let response = await new CurrencyConversionRateModel().getLastActiveRates(
        configStrategy.OST_UTILITY_CHAIN_ID,
        currencyCode
      );
