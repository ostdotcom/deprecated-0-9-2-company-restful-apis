...
const EthAddrPrivateKeyCacheKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    return Promise.resolve(
      responseHelper.successWithData({
        ...
        client_id: managedAddress.client_id
      })
    );
  },
  ...
  fetchDecryptedData: async function() {
    ...
    if (cachedResponse['client_id'] === null) {
      cachedResponse['client_id'] = '0';
    }

    let addressEncryptorClassObject = oThis.ic().getAddressEncryptorKlass();

    let encrObj = new addressEncryptorClassObject({ managedAddressSaltId: cachedResponse.managed_address_salt_id });

    return Promise.resolve(
      responseHelper.successWithData({
        private_key_d: await encrObj.decrypt(cachedResponse['private_key_e']),
        client_id: cachedResponse['client_id']
      })
    );
  }
};
