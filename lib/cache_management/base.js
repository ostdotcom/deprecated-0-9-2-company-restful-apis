...
const BaseCacheManagementKlass = function(params) {
  ...
  if (oThis.cacheType !== cacheManagementConst.memcached) {
    throw `cache_management: Invalid cache type: ${oThis.cacheType}`;
  }

  // We are passing cacheType now because we might need to add some other client specific cache provider at a later stage.
  ...
  _setCache: function(dataToSet) {
    ...
    setCacheFunction().then(function(cacheSetResponse) {
      if (cacheSetResponse.isFailure()) {
        notifier.notify('cm_b_2', 'Something Went Wrong', cacheSetResponse);
      }
    });
  },
