...
const _private = {
  ...
  mandatoryParamsPresent: function(inputParams){
    if(!inputParams["signature"] || !inputParams["request_timestamp"] || !inputParams["api_key"]){
      return Promise.resolve(responseHelper.error("a_vs_1", "Mandatory parameters missing.", null, [],
  ...
  validateRequestTime: function(requestTime){
    ...
    if(currentTime > (parseInt(requestTime) + 10)){
      return Promise.resolve(responseHelper.error("a_vs_2", "Request Expired.", null, [],
        {sendErrorEmail: false}));
    }
    return Promise.resolve(responseHelper.successWithData());
  },
  ...
  validateParams: async function(inputParams, reqUrl){
    ...
    if(clientKeys.data['expiryTimestamp'] < currentTimeStamp){
      return Promise.resolve(responseHelper.error("a_vs_3", "Client api credentials expired. Contact support to generate new one."));
    }
    ...
    if(computedSignature != signature){
      return Promise.resolve(responseHelper.error("a_vs_6", "Invalid Signature.", null, [],
        {sendErrorEmail: false}));
    }
