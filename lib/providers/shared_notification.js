...
const rootPrefix = '../..',
  ...
  ConfigStrategyHelper = require(rootPrefix + '/helpers/config_strategy/by_group_id'),
  ...
  getSharedRmqData: async function() {
    ...
    if (!oThis.sharedRmqData) {
      let configStrategyHelperObj = new ConfigStrategyHelper(),
        sharedRmqResponse = await configStrategyHelperObj.getForKind(configStrategyConstants.shared_rmq);

      if (sharedRmqResponse) {
        oThis.sharedRmqData = Object.values(sharedRmqResponse.data)[0];
      } else {
