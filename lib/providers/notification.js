...
NotificationProviderKlass.prototype = {
  ...
  getInstance: async function(params) {
    ...
    let instance = await oThis.getNotiInstance();

    return instance;
  },

  getNotiInstance: async function() {
    const oThis = this,
      configStrategy = oThis.ic().configStrategy;

    let instance = await OpenStNotification.getInstance(configStrategy);

    let connection = await instance.connection.get();

    if (!connection) {
      //reload config strategy
      configStrategy = oThis.ic().configStrategy = configStrategy;
      // get instance again
      setImmediate(function() {
        oThis.getNotiInstance(configStrategy);
      });
    } else {
      return instance;
    }
...
process.on('ost_rmq_fail', function(msg) {
  console.log(msg);
  // check and change all config strategies having failed host.
});
