...
UserAirdropContractApproveKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(function (error) {
        if (responseHelper.isCustomResult(error)) {
          ...
          return responseHelper.error("l_am_dt_uaca_1", "Unhandled result", null, {}, {sendErrorEmail: false});
        }
      });
  },
  ...
  _fetchClientAirdropRecord: async function () {
    ...
    if (!oThis.clientAirdrop) {
      return Promise.reject(responseHelper.error('l_am_dt_uaca_2', 'client airdrop not found.', null, {}, {sendErrorEmail: false}));
    }

    if (oThis.clientAirdrop.client_branded_token_id != oThis.clientBrandedTokenId) {
      return Promise.reject(responseHelper.error('l_am_dt_uaca_3', 'invalid task params', null, {}, {sendErrorEmail: false}));
    }
  ...
  _fetchClientBrandedTokenRecord: async function () {
    ...
    if (!oThis.clientBrandedToken) {
      return Promise.reject(responseHelper.error('l_am_dt_uaca_4', 'client branded token not found.', null, {}, {sendErrorEmail: false}));
    }
  ...
  _fetchReserveAddressUuid: async function () {
    ...
    if (!oThis.reserveAddressUuid) {
      return Promise.reject(responseHelper.error('l_am_dt_uaca_5', 'reserveAddressUuid not found.', null, {}, {sendErrorEmail: false}));
    }
      ...
      promiseArray.push(
        new Promise(async function (onResolve, onReject) {
          ...
          await oThis._createTransactionLog(
            uuid.v4(),
            refillGasResponse.data['input_params'],
            refillStatus, error, transactionHash);
    ...
    for (var i = 0; i < fromUuids.length; i++) {
      ...
      promiseArray.push(
        new Promise(async function (onResolve, onReject) {
          ...
          await oThis._createTransactionLog(
            uuid.v4(),
            approveResponse.data['input_params'],
            approveStatus,
            error,
   ...
   * @return {promise<result>}
   */
  _createTransactionLog: function (uuid, inputParams, status, responseData, transactionHash) {
    const oThis = this
      , ipp = JSON.stringify(inputParams)
    ...
    return new TransactionLogModel().insert({
      client_id: oThis.clientAirdrop.client_id,
