...
contractApproveKlass.prototype = {
  perform: async function() {
    ...
    oThis.clientBrandedTokenObj = await clientBrandedTokenCacheObj.fetch();

    // decrypt reserve passphrase
    const managedAddressSqlResponse = await new ManagedAddressModel().getByIds([
        oThis.clientBrandedTokenObj.data.airdrop_holder_managed_address_id
      ]),
    ...
    const airdropManagerApproveObj = new openSTPayments.services.airdropManager.approve({
      airdrop_contract_address: oThis.clientBrandedTokenObj.data.airdrop_contract_addr,
