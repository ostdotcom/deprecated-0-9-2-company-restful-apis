...
baseKlass.prototype = {
  ...
  _checkReserveAddressBalance: async function() {
    ...
    const brandedTokenQueryResponse = await new ClientBrandedTokenModel().getById(
      oThis.clientAirdropObj.client_branded_token_id
    );
    oThis.clientBrandedTokenObj = brandedTokenQueryResponse[0];

    // get reserved address details
    const managedAddressQueryResponse = await new ManagedAddressModel().getByIds([
    ...
    const fetchBalanceObj = new openSTPlaform.services.balance.brandedToken({
      address: managedAddressObj.ethereum_address,
      erc20_address: oThis.clientBrandedTokenObj.token_erc20_address
    });
    ...
    if (totalBalanceInWeiBignNumber.lessThan(airdropAmountInWeiBignNumber.times(totalCountBignNumber))) {
      logger.error(
        'am_dt_ta_b_crab_3',
        'Insufficient Balance of Reserved Address for airdropID - ',
        oThis.clientAirdropObj.id
      );
      return Promise.reject(
        responseHelper.error({
    ...
    while (true) {
      ...
      for (var i = 0; i < totalCount; i++) {
        const sql_rows = [
          ...
          oThis.clientAirdropObj.id,
          results[i].id,
          oThis.clientAirdropObj.common_airdrop_amount_in_wei,
          oThis.clientAirdropObj.common_expiry_timestamp,
...
InstanceComposer.registerShadowableClass(baseKlass, 'getToAddressesBaseClass');
