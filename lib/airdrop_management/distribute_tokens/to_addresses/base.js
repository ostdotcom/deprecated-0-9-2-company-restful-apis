...
baseKlass.prototype = {
  ...
  _checkReserveAddressBalance: async function () {
    ...
    const totalBalanceBignNumber = balanceResponse.data.balances[oThis.clientBrandedTokenObj.symbol];

    var results = await managedAddress.getFilteredActiveUsersCount({client_id: oThis.client_id, property_unset_bit_value: oThis.propertyUnsetBitValue})
      , totalCount = results[0].total_count
      , airdropAmountInWei = oThis.clientAirdropObj.common_airdrop_amount_in_wei
    ;

    const totalBalanceInWeiBignNumber = basicHelper.convertToWei(totalBalanceBignNumber)
  ...
  _createAirdropDetails: async function () {
    ...
    while (continueLoop) {
      ...
      for (var i = 0; i < totalCount; i++) {
        var sql_rows = [oThis.client_id, oThis.clientAirdropObj.id, results[i].id, oThis.clientAirdropObj.common_airdrop_amount_in_wei, oThis.clientAirdropObj.common_expiry_timestamp, clientAirdropDetail.invertedStatuses[clientAirdropDetailsConst.incompleteStatus]];
