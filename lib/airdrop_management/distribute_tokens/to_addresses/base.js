...
baseKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function(error) {
        ...
        if (responseHelper.isCustomResult(error)){
          ...
          return Promise.resolve(responseHelper.error("am_dt_s_p_1", "Unhandled result", [error], {}));
        }
      });
  },
  ...
  _checkReserveAddressBalance: async function () {
    ...
    if (parseInt(totalCount) === 0){
      ...
      return Promise.reject(responseHelper.error("am_dt_ta_b_crab_2", "No users found for airdrop list"));
    }
    ...
    if (totalBalanceInWeiBignNumber.lessThan(airdropAmountInWeiBignNumber.times(totalCountBignNumber))) {
      ...
      return Promise.reject(responseHelper.error('am_dt_ta_b_3', 'Insufficient Balance for airdrop'));
    }
