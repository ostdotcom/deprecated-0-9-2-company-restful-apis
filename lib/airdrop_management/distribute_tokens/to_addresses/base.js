...
baseKlass.prototype = {
    ...
    while (true) {
      ...
      for (var i = 0; i < totalCount; i++) {
        const sql_rows = [oThis.client_id, oThis.clientAirdropObj.id, results[i].id
          ...
          , new ClientAirdropDetailKlass().invertedStatuses[clientAirdropDetailsConst.incompleteStatus]];
  ...
  _bulkInsertClientAirdropDetails: async function (sql_rows_array) {
    ...
    await new ClientAirdropDetailKlass().insertMultiple(fields, sql_rows_array).fire();
    return Promise.resolve(responseHelper.successWithData({}));
  }
