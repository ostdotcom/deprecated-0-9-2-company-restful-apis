...
contractApproveKlass.prototype = {
  ...
  perform: async function () {
    ...
    while (true) {
      ...
      const clientAirdropDetail = new ClientAirdropDetailKlass()
      ...
      const results = await clientAirdropDetail.getPendingRecordsInbatches(oThis.clientAirdropId, limitSize, offset)
      ...
      const response = await openStPaymentsAirdropManager.batchAllocate(
        oThis.clientBrandedToken.airdrop_contract_addr,
  ...
  _setPropertiesForMangedAddress: async function (managedAddressIds) {
    ...
    await managedAddress.update(['properties = properties | ?', managedAddress.invertedProperties[managedAddressesConst.airdropGrantProperty]]).where(['id IN (?)', managedAddressIds]).fire();
