...
contractApproveKlass.prototype = {
    ...
    return oThis.asyncPerform()
      .catch(async function(error) {
        ...
        if (responseHelper.isCustomResult(error)){
          ...
          return Promise.resolve(responseHelper.error("am_dt_at_1", "Unhandled result", [error], {}));
        }
      });
  },
  ...
  asyncPerform: async function () {
    ...
    while (true) {
      ...
      for (var i = 0; i < totalCount; i++) {
        ...
        if (!new ManagedAddressModel().isBitSet(managedAddressesConst.airdropGrantProperty, managedAddressObj.properties)) {
  ...
  _setPropertiesForMangedAddress: async function (managedAddressIds) {
    ...
    await new ManagedAddressModel().update(['properties = properties | ?',
      new ManagedAddressModel().invertedProperties[managedAddressesConst.airdropGrantProperty]])
      .where({id: managedAddressIds}).fire();
  ...
  _updateAirdropDetailsStatus: async function (airdropDetailsIds, status) {
    ...
    await new ClientAirdropDetailModel().update({status: statusDb}).where(['id in (?)', airdropDetailsIds]).fire();
