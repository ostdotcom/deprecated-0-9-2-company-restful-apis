...
startKlass.prototype = {
  ...
  approveReserveForAirdropContract: async function() {
    ...
    // fetch branded token details
    const clientBrandedTokenObj = await clientBrandedTokenCacheObj.fetch();
    ...
    const managedAddressSqlResponse = await new ManagedAddressModel()
      .select('*')
      .where(['id=?', clientBrandedTokenObj.data.reserve_managed_address_id])
      .fire();
    ...
    let inputParams = {
      ...
      token_erc20_address: clientBrandedTokenObj.data.token_erc20_address,
      approvee_address: clientBrandedTokenObj.data.airdrop_contract_addr,
