...
startKlass.prototype = {
  ...
  markCriticalLogFailure: async function (errResponse) {
    ...
    return await new CriticalChainInteractionLogModel()
      .update(
        {
      ...
      .where(['id=?', oThis.criticalInteractionLogId]).fire();
  },
  ...
  _setClientAirdropObj: async function () {
    ...
    await new ClientAirdropModel()
      ...
      .where('id=?', oThis.clientAirdropObj.id).fire();

    await new CriticalChainInteractionLogModel()
      .update('status=?', oThis.criticalChIntLogObj.invertedStatuses[criticalChainInteractionLogConst.pendingStatus])
      .where('id=?', oThis.clientAirdropObj.id).fire();
  ...
  _markCompleteStatus: async function () {
    ...
    await new CriticalChainInteractionLogModel()
      .update({status: oThis.criticalChIntLogObj.invertedStatuses[criticalChainInteractionLogConst.processedStatus]})
      .where('id=?', oThis.clientAirdropObj.id).fire();
  ...
  _updateFailedStatusClientAirdropObj: async function (responseData) {
    ...
    await new CriticalChainInteractionLogModel()
      .update({
        status: oThis.criticalChIntLogObj.invertedStatuses[criticalChainInteractionLogConst.failedStatus],
  ...
  approveReserveForAirdropContract: async function(){
    ...
    const clientBrandedToken = new ClientBrandedTokenKlass()
      ...
      , clientBrandedTokenSqlResponse = await clientBrandedToken.getById(oThis.clientAirdropObj.client_branded_token_id);
    var clientBrandedTokenObj = clientBrandedTokenSqlResponse[0];


    const managedAddressSqlResponse = await managedAddress.getByIds([clientBrandedTokenObj.reserve_managed_address_id])
