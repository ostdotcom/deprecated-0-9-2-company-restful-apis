...
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/token_transfer');
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/contract_approve');
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/allocate_token');
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/to_addresses/all');
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/to_addresses/never_airdropped');
require(rootPrefix + '/lib/airdrop_management/distribute_tokens/to_addresses/ever_airdropped');
...
startKlass.prototype = {
  ...
  _setClientAirdropObj: async function() {
    ...
    const airdrop_result = await new ClientAirdropModel()
      .select('*')
      .where({ id: oThis.clientAirdropId })
      .fire();
    oThis.clientAirdropObj = airdrop_result[0];
    oThis.clientId = oThis.clientAirdropObj.client_id;

    if (
      oThis.clientAirdropObj.status !==
      parseInt(new ClientAirdropModel().invertedStatuses[clientAirdropConst.incompleteStatus])
    ) {
      notifier.notify(
        'am_dt_s_scao_1',
    ...
    if (oThis.criticalInteractionLogId) {
      ...
      const criticalInteractionLogRecord = await new CriticalChainInteractionLogModel()
        .select('*')
        .where({ id: oThis.criticalInteractionLogId })
        .fire();
  ...
  _addClientAirdropDetails: async function() {
    const oThis = this,
      AllAddressesKlass = oThis.ic().getAllAddressesClass(),
      NeverAirdroppedAddressesKlass = oThis.ic().getNeverAirdroppedClass(),
      EverAirdroppedAddressesKlass = oThis.ic().getEverAirdroppedClass();

    if (
      new ClientAirdropModel().isBitSet(
        clientAirdropConst.usersIdentifiedStepComplete,
        oThis.clientAirdropObj.steps_complete
      )
    ) {
      notifier.notify(
        'am_s_1',
        'Invalid state for airdrop. user_identified step already done',
        {},
        { client_airdrop_id: oThis.clientAirdropObj.id }
      );
    ...
    let airdropToAddressesKlass = null;
    if (
      oThis.clientAirdropObj.airdrop_list_type &
      parseInt(new ClientAirdropModel().invertedAirdropListType[clientAirdropConst.allAddressesAirdropListType])
    ) {
      airdropToAddressesKlass = AllAddressesKlass;
    } else if (
      oThis.clientAirdropObj.airdrop_list_type &
      parseInt(
        new ClientAirdropModel().invertedAirdropListType[clientAirdropConst.neverAirdroppedAddressesAirdropListType]
      )
    ) {
      ...
      oThis.clientAirdropObj.airdrop_list_type &
      parseInt(
        new ClientAirdropModel().invertedAirdropListType[clientAirdropConst.everAirdroppedAddressesAirdropListType]
      )
    ) {
    ...
    const airdropToAddresses = new airdropToAddressesKlass({
        client_airdrop_obj: oThis.clientAirdropObj,
        uuids: oThis.userIds
      }),
  ...
  _startTokenTransfer: async function() {
    const oThis = this,
      TokenTransferKlass = oThis.ic().getTokenTransferClass(),
  ...
  _startContractApprove: async function() {
    const oThis = this,
      ContractApproveKlass = oThis.ic().getContractApproveClass(),
  ...
  _allocateTokenToAddresses: async function() {
    const oThis = this,
      AllocateTokenKlass = oThis.ic().getAllocateTokenClass(),
  ...
  _updateFailedStatusClientAirdropObj: async function(responseData) {
    const oThis = this,
      response = responseData.getDebugData();

    response.err.debugOptions = response.err.debugOptions || {};
    response.err.debugOptions.tokens_transfered_transaction_hash = oThis.tokensTransferedTransactionHash;
    response.err.debugOptions.contract_approve_transaction_hash = oThis.contractApproveTransactionHash;
  ...
  approveReserveForAirdropContract: async function() {
    ...
    const clientBrandedTokenObj = clientBTCacheResponse.data;
    ...
    const managedAddressSqlResponse = await new ManagedAddressModel()
      .select('*')
      .where(['id=?', clientBrandedTokenObj.reserve_managed_address_id])
      .fire();

    const managedAddressObj = managedAddressSqlResponse[0];

    let inputParams = {
      approverUuid: managedAddressObj.uuid,
      token_erc20_address: clientBrandedTokenObj.token_erc20_address,
      approvee_address: clientBrandedTokenObj.airdrop_contract_addr,
  ...
  enqueueUserApprovalJob: async function() {
    ...
    await openSTNotification.publishEvent
      ...
      .catch(function(err) {
        logger.error('Message for airdrop approval was not published. Payload: ', payload, ' Error: ', err);
      });
...
InstanceComposer.registerShadowableClass(startKlass, 'getDistributeTokensStartClass');
