...
tokenTransferKlass.prototype = {
  perform: async function() {
    ...
    // Get total tokens to be transferred from reserve to budget holder
    ...
    if (response.isFailure()) {
      notifier.notify('am_dt_tt_p_2', 'Error in tokenTransfer', response, { clientAirdropId: oThis.clientAirdropId });
    }
  ...
  _checkAndRefillSTPrimeBalance: async function() {
    ...
    if (fundResponse.isFailure()) {
      notifier.notify('am_dt_tt_carstpb_1', 'Error in checkAndRefillSTPrimeBalance', fundResponse, {
