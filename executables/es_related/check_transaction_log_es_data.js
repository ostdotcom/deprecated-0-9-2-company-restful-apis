...
CheckTransactionLogESData.prototype = {
  ...
  checkForESRecords: function() {
    ...
    if (!table) return;
    oThis
      .validateDynamoRecordsForTable(table)
      .then(function(response) {
        ...
        if (shardCount < len) {
          ...
          logger.win(
            'SUCCESS - DynamoDB fetch for tables' +
              tables.join(' , ') +
              ' completed with total records - ' +
              oThis.totalRecordCnt
          );
        }
      })
      .catch(function(reason) {
        logger.error(
          ' REJECT - DynamoDB fetch for ' + oThis.getTableName() + ' for batch ' + oThis.shardBatchCnt + ' failed! ',
          reason
        );
      });
  },
    ...
    return new Promise(function(resolve, reject) {
      function checkDynamoRecordsInES(LastEvaluatedKeyHash) {
        ...
        ddbServiceObj
          ...
          .then(function(response) {
            ...
            oThis
              .validateRecordsInES(items)
              .then(function(response) {
                logger.win(
                  ' WIN - ES validation for DynamoDB table ' +
                    oThis.getTableName() +
                    ' for batch ' +
                    oThis.shardBatchCnt +
                    ' complete!'
              ...
              .catch(function(reason) {
                logger.error(
                  ' ERROR - ES validation for DynamoDB table ' +
                    oThis.getTableName() +
                    ' for batch ' +
                    oThis.shardBatchCnt +
                    ' failed!',
      ...
      function onESValidation(LastEvaluatedKeyHash) {
        ...
        if (LastEvaluatedKeyValue) {
          ...
          logger.win(
            ' WIN - DynamoDB data validation for table ' +
              oThis.getTableName() +
              ' completed with records ' +
              oThis.shardRecordCnt
          );
