...
FixStatusKlass.prototype = {
  ...
  perform: async function () {
    ...
    const oThis = this
        ...
        , inactiveStatus = new ClientWorkerManagedAddressIdModel().invertedStatuses[clientWorkerManagedAddressConst.inactiveStatus]
    ;
    ...
    while (true) {
      ...
      var dbRows = await new ClientWorkerManagedAddressIdModel().select('id, managed_address_id, status').where(['id >= ?', startId]).limit(pageLimit).fire();
      ...
      var managedAddressRows = await new ManagedAddressModel().select('id, ethereum_address').where(['id in (?)', Object.keys(managedAddressIdDataMap)]).fire();
      ...
      for(var i=0; i<managedAddressRows.length; i++) {
        ...
        // console.log(rsp);
        if (!rsp.data.isValid) {
          console.error("problem with: ethereum_address: ", managedAddressRow.ethereum_address, " idToUpdate: ",  idToUpdate);
